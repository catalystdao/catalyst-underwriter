...
export abstract class ProcessingQueue<OrderType, ReturnType=OrderType> {
    ...
    protected async onProcessOrders(
    ): Promise<void> {};

    protected async onProcessRetries(
    ): Promise<void> {};
    ...
    async processOrders(): Promise<void> {
        ...
        for (const order of this.ordersQueue) {
            try {
                const handleResult = await this.handleOrder(order.order, order.retryCount);
    ...
    private async handleOrderSuccess(order: ProcessOrder<OrderType>, result: ReturnType | null): Promise<void> {
    ...
    private async handleOrderError(order: ProcessOrder<OrderType>, error: any, errorOnPending: boolean): Promise<void> {
        ...
        const retryOrder = await this.handleFailedOrder(order.order, order.retryCount, error, errorOnPending);
        if (retryOrder) {
